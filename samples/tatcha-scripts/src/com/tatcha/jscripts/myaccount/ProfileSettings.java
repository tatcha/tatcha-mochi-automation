...
public class ProfileSettings {
	 ...
	 * @throws Exception
	 */
	public void verifyProfileSettings(String FLOW_ID, WebDriver driver, Properties prop, Properties locator,
			Properties data, User user, List<TestCase> tcList) throws Exception {

		logger.info("BEGIN verifyProfileSettings");
		final String FUN_ID = "FUN_VPS";
...
//		testCase = new TestCase("TC-21.1", "MOC-NIL", FUNCTIONALITY, "FAIL", "");
		...
		if (getTestHelper().isLoggedIn(driver)) {
			...
			getTestHelper().logAssertion(getClass().getSimpleName(), prop.getProperty("myaccount.item1").toString(),
					webElement.getText());
			...
			getTestHelper().logAssertion(getClass().getSimpleName(), prop.getProperty("profile.accountInfo").toString(),
					accountInfoElement.getText());
			...
			getTestHelper().logAssertion(getClass().getSimpleName(), user.getEmail(),
					accountInfoElement.getText().substring(0, emailLength));
			...
			getTestHelper().logAssertion(getClass().getSimpleName(), prop.getProperty("profile.profileInfo").toString(),
					profileInfoElement.getText());
			...
			getTestHelper().logAssertion(getClass().getSimpleName(), name,
					profileInfoElement.getText().substring(0, length).toUpperCase());

			// Change password
			driver.findElement(By.xpath(locator.getProperty("profile.accountInfo.password.link").toString())).click();
			changePassword(FLOW_ID, driver, data, locator, user, tcList);
			...
			changeEmail(FLOW_ID, driver, data, locator, tcList);
			wait.until(ExpectedConditions.visibilityOfElementLocated(By.cssSelector("h1.text-center")));

			// Edit profile information
			driver.findElement(By.xpath(locator.getProperty("profile.profileInfo.edit").toString())).click();
			editProfile(FLOW_ID, driver, data, locator, tcList);
			wait.until(ExpectedConditions.visibilityOfElementLocated(By.cssSelector("h1.text-center")));
		}
	 ...
	 * @param user
	 */
	public void changePassword(String FLOW_ID, WebDriver driver, Properties prop, Properties locator, User user, List<TestCase> tcList)
			throws Exception {
		
		logger.info("BEGIN changePassword");
		final String FUN_ID = "FUN_CPW";
...
//		testCase = new TestCase("TC-21.2", "MOC-NIL", FUNCTIONALITY, "FAIL", "");
	 ...
	 * @param locator
	 */
	public void changeEmail(String FLOW_ID, WebDriver driver, Properties prop, Properties locator, List<TestCase> tcList)
			throws Exception {
		logger.info("BEGIN changeEmail");
		final String FUN_ID = "FUN_CEM";
...
//		testCase = new TestCase("TC-21.3", "MOC-NIL", FUNCTIONALITY, "FAIL", "");
	 ...
	 * @throws Exception
	 */
	public void editProfile(String FLOW_ID, WebDriver driver, Properties prop, Properties locator, List<TestCase> tcList)
			throws Exception {
		logger.info("BEGIN editProfile");
		final String FUN_ID = "FUN_EPR";
...
//		testCase = new TestCase("TC-21.4", "MOC-NIL", FUNCTIONALITY, "FAIL", "");
		...
		if (saveBirthday && monthEnabled && dayEnabled) {
			...
			wait.until(ExpectedConditions.visibilityOf(
					driver.findElement(By.xpath(locator.getProperty("confirmDob.modal.title").toString()))));
			...
			wait.until(ExpectedConditions
					.visibilityOf(driver.findElement(By.xpath(locator.getProperty("profile.title").toString()))));
		...
		/*
		 * WebElement alertMessageElement = null; try { alertMessageElement =
		 * driver.findElement(By.className("alert alert-danger")); WebElement
		 * cancelButtonElement = driver .findElement(By.className(
		 * "btn btn-default edit-cancel account-switch-section"));
		 * cancelButtonElement.click(); } catch (NoSuchElementException ne) {
		 * assertTrue(alertMessageElement == null); }
		 */
